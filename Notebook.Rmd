...
misstest <- as.data.frame(ifelse(sapply(data, is.na),1,0))
misstest$id_hogar <- data$id_hogar
# assign the missingness values to one variable so that any observation with a 1 gets a 1, and any observation that is all 0s get a 0
misstest %>% 
  group_by(id_hogar) %>% summarize(miss = n())
